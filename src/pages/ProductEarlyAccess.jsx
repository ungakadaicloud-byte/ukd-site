...
export default function ProductEarlyAccess() {
  const { slug } = useParams()
  const product = getProduct(slug)
  if (!product || product.status !== 'early') return <Navigate to="/" replace />
      ...
      <section id="demo" className="border-t border-ink-border">
        <div className="max-w-lg mx-auto px-6 py-20">
          ...
          <p className="text-sm text-paper/50 text-center mb-8">
            Be first to know when {product.name} opens up — no spam, just one message when it's ready.
          </p>
          <LeadForm
            product={product}
